...
const TIMER_CONFIG = {
  duration: 3 * 60 * 60 * 1000, // 3 часа в миллисекундах
  storageKey: "promoTimerEndTime",
  urgentThreshold: 60 * 60 * 1000, // 1 час в миллисекундах
}
...
class PromoTimer {
  ...
  loadOrCreateTimer() {
    const savedEndTime = localStorage.getItem(TIMER_CONFIG.storageKey)
    const now = Date.now()

    if (savedEndTime) {
      this.endTime = Number.parseInt(savedEndTime)

      // Если таймер истек, создаем новый
      if (this.endTime <= now) {
        this.createNewTimer()
      }
    } else {
  ...
  createNewTimer() {
    this.endTime = Date.now() + TIMER_CONFIG.duration
    localStorage.setItem(TIMER_CONFIG.storageKey, this.endTime.toString())
    console.log("Создан новый таймер на", new Date(this.endTime))
  }
  ...
  updateDisplay() {
    ...
    const time = this.calculateTime(timeLeft)
    this.renderTime(time)
    this.checkUrgentState(timeLeft)
  }
  ...
  checkUrgentState(timeLeft) {
    const timeBlocks = document.querySelectorAll(".time-block")

    if (timeLeft <= TIMER_CONFIG.urgentThreshold) {
      timeBlocks.forEach((block) => block.classList.add("urgent"))
    } else {
      timeBlocks.forEach((block) => block.classList.remove("urgent"))
    }
  }

  onTimerEnd() {
    console.log("Таймер завершен, создаем новый")

    // Показываем анимацию завершения
    this.showCompletionAnimation()

    // Создаем новый таймер через небольшую задержку
    setTimeout(() => {
      this.createNewTimer()
      this.updateDisplay()
    }, 2000)
  ...
  showCompletionAnimation() {
    ...
    timeBlocks.forEach((block, index) => {
      setTimeout(() => {
        ...
        block.style.background = "rgba(255, 215, 0, 0.3)"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.getElementById("countdown-timer")) {
    ...
    localStorage.removeItem(TIMER_CONFIG.storageKey);
...
window.addEventListener("beforeunload", () => {
  if (window.promoTimer) {
    window.promoTimer.destroy()
  }
})

// Дополнительные функции для управления таймером (для отладки)
window.resetPromoTimer = () => {
  localStorage.removeItem(TIMER_CONFIG.storageKey)
  if (window.promoTimer) {
    window.promoTimer.destroy()
    window.promoTimer = new PromoTimer()
...
window.setPromoTimer = (hours) => {
  const newEndTime = Date.now() + hours * 60 * 60 * 1000
  localStorage.setItem(TIMER_CONFIG.storageKey, newEndTime.toString())
  if (window.promoTimer) {
    window.promoTimer.endTime = newEndTime
  }
  console.log(`Таймер установлен на ${hours} часов`)
}